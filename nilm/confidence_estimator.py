...
def changed_devices(devices, on_off, i):
    """ Returns all devices whose I/O state changed between at a single time period i. """

    return [x for x in devices if on_off[i, x] != on_off[i-1, x]]

def get_changed_data(devices, time_series, on_off):
    # Generates data for each device by the step
    # inference method, calculating the change in
    # energy usage as a single device changes.
    ...
    for i in range(2,len(time_series)):
        ...
        if (len(changed) == 1): data[changed[0]].append(time_series[i] - time_series[i-1])
...
def confidence_estimator(time_series, on_off, devices, data_sorter):
    """
        ...
        with on/off indicators (dictionary of (time, device) pairs mapping to power,
        computes the best time estimators by the confidence interval subtraction method.
        Data obtained for the confidence interval measure is obtained via the
        data_sorter function, which is currently implemented to either take samples
        from immediate changes in power from a single device switching on
        (get_changed_data) or taking samples from when a single device is on (sort_data).
        The program assumes that the time data between the series and indicators is the
        ...
        point. If not, this function is not be able to estimate the programs accurately.
        get_changed_data assumes that the data is linearly ordered (time is integer valued),
        whereas sort_data can have any time representation.
     """
    ...
    else: mean_choice = sum(data[choice])/len(data)
    ...
    calculated_means = confidence_estimator(new_series, on_off, new_devices, data_sorter)
