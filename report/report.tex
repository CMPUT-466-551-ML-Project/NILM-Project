...
For example, \cite{Kolter} used one Hidden Markov Model per device to describe its consumption and figure out what time it was active at.
Kolter's algorithm finds periods of activity, clusters similar ones and builds one hidden Markov model per device \cite{Kolter}.
Unfortunately the process to find these periods is not described in detail but seems to look for intervals when only one device is active.
...
In contrast to this completely unsupervised approach, Parson starts with knowledge about the general device type and produces a generic model for each device, which is fitted to the actual device.
...
Even though his allocation of generic models and minute-wise measurements did not seem useful for us, his work led us to the REDD-dataset which he used and to the idea of looking for timesteps when only one device changes its activity status.
...
%TODO: Add label? and such. This needs to be at top of page.....
$$\begin{tabular}{c||c|c|c|c|c|}
Device & Total RMSE & f\_score & Activation RMSE & Maximum Energy usage & \% time active\\ %TODO: Add recall, precision?
\hline
kitchen outlets & 71.78 & 0.6087 & 79.49  & 1582.0 & 72.2\% \\ % Is house 3 channel 7
\hline
washer \& dryer & 209.3 & 0.233 & 1657.61  & 3222.5 & 1.4\% \\ % Is house 1 channel 20
\hline
\end{tabular}$$
\captionof{table}{Table Title}\label{tab:RawEstimate} 
\end{table*}

%TODO: Fill in the data for this
\begin{table*}[t]
	\centering
	...
	Device & Total RMSE & f\_score & Activation RMSE & Maximum Energy usage & \% time active\\ %TODO: Add recall, precision?
	\hline
	kitchen outlets & &  &  & &  \\ % Is house 3 channel 7
	\hline
	washer \& dryer & &  &  & &  \\ % Is house 1 channel 20
	\hline
	\end{tabular}$$
	\captionof{table}{Table Title}\label{tab:RawEstimateAfterGenerate} 
\end{table*}
...
\hline
\end{tabular}$$
\captionof{table}{Table Title}\label{tab:PreprocessVsRawRMSE}
\end{table}
